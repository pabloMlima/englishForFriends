...
class ForumModel extends Model
{
    ...
    public function visualizaConteudo($dados){
        $busca = DB::table('forum_conteudos AS f_c')
                            ->leftjoin('forum_tipos AS f_t', 'f_c.forum_tipos_id', '=', 'f_t.forum_tipos_id')
                            ->leftjoin('usuarios AS u', 'u.usuarios_id', '=', 'f_c.usuarios_id')
                            ->select('f_c.for_con_texto', 'f_c.for_con_traducao', 'f_c.forum_tipos_id', 'f_c.for_con_link',
    ...
    public function insereFavoritos($dados){
        $insere = DB::table('usuarios_forum_favoritos')
                            ->insert([
                                'usuarios_id' => $dados['usuarios_id'],
                                'forum_conteudos_id' => $dados['forum_conteudos_id']
                            ]);
        return $insere;
    }
    public function buscaFavoritos($dados){
        $busca = DB::table('usuarios_forum_favoritos')
                            ->where('forum_conteudos_id', $dados['forum_conteudos_id'])
                            ->get();
        return $busca;
    }
    public function deleteFavoritos($dados){
        $busca = DB::table('usuarios_forum_favoritos')
                            ->where('forum_conteudos_id', $dados['forum_conteudos_id'])
                            ->delete();
        return $busca;
    }
    public function deleteForumConteudos($dados){
        $update = DB::table('forum_conteudos')
                            ->where('forum_conteudos_id', $dados['forum_conteudos_id'])
